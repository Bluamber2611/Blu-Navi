...
bubble = "on-bubble" if paper_mode else "off-bubble"
st.sidebar.markdown(f'<div class="toggle-label"><span class="toggle-bubble {bubble}"></span>Paper Mode</div>', unsafe_allow_html=True)
...
def plot_chart(data, signal, auto_draw):
    fig = go.Figure()
    fig.add_trace(go.Candlestick(x=data.index, open=data['Open'], high=data['High'], low=data['Low'], close=data['Close'], name='Gold'))
    ...
    if signal and auto_draw:
        fig.add_trace(go.Scatter(x=[signal['time']], y=[signal['price']], mode='markers', marker=dict(symbol='triangle-up', size=18, color='lime'), name='BUY'))
...
with col2:
    ...
    if signal:
        ...
        if balance < risk:
            st.error(f"Need ${risk:.0f}, have ${balance:.0f}")
        else:
            c1, c2 = st.columns(2)
            with c1:
